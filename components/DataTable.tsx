...
export default function DataTable({ data, columns, primaryKey, tableName, onRefresh }: DataTableProps) {
  ...
  const handleSave = async (rowData: any) => {
    ...
    const url = isEditing ? `/api/crud/${tableName}/${selectedRow[primaryKey[0]]}` : `/api/crud/${tableName}`;
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        const response = await fetch(`/api/crud/${tableName}/${row[primaryKey[0]]}`, {
          method: 'DELETE',
        });
